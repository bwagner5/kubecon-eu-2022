...
IFS=$'\n'
while true; do 
    for line in $(kubectl get nodes --no-headers --selector node.kubernetes.io/instance-type --selector karpenter.sh/capacity-type -L node.kubernetes.io/instance-type -L karpenter.sh/capacity-type -L topology.kubernetes.io/zone -L node.k8s.aws/spot-percent-discount | tr -s " "); do
        node=$(echo ${line} | cut -d' ' -f1)
        instanceType=$(echo ${line} | cut -d' ' -f6)
        capacityType=$(echo ${line} | cut -d' ' -f7)
        zone=$(echo ${line} | cut -d' ' -f8)
        spotDiscount=$(echo ${line} | cut -d' ' -f9)
        if [[ ${capacityType} = "spot" ]] && [[ -z ${spotDiscount} ]]; then 
